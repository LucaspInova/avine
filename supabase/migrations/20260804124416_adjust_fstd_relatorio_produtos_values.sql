-- Ajusta os valores do relatorio para refletir cada produto da NFD.
-- O valor deixa de repetir o total da nota e passa a ser:
-- Valor = VL Galinha + VL Codorna do produto correspondente.
...
with produto_motivos as (
  select
    ...
    coalesce(fp.quantidade_retorno, 0) as retorno_produto,
...
produto_motivo_escolhido as (
  select
    pm.*,
    row_number() over (
      partition by pm.produto_id
      order by
        pm.quantidade_faturada_motivo desc,
        pm.quantidade_retorno_motivo desc,
        pm.motivo_ordem asc nulls last,
        pm.motivo_nome asc nulls last,
        pm.motivo_id
    ) as ordem_motivo
  from produto_motivos as pm
)
select
  ...
  round(
    coalesce(
      case
        when coalesce(ni.quantidade_galinha, 0) > 0
          then coalesce(ni.valor_galinha, 0)
        else 0
      end,
      0
    )
    + coalesce(
      case
        when coalesce(ni.quantidade_codorna, 0) > 0
          then coalesce(ni.valor_codorna, 0)
        else 0
      end,
      0
    ),
...
from public.fstd_processos as p
join produto_motivo_escolhido as pm
  on pm.processo_id = p.id
 and pm.ordem_motivo = 1
...
comment on view public.fstd_relatorio_produtos is
  'Relatorio de FSTDs concluidas com uma linha por produto, motivo dominante e valores especificos do produto.';
