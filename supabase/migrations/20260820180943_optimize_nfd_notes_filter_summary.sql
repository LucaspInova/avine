-- The notes grid does not consume the JSON item details exposed by nfd_notas.
-- Reading that view forced PostgreSQL to build details for every matching note
-- before pagination, which made broad filters exceed the RPC timeout. Aggregate
-- only the columns rendered in the grid and retain the same RLS-protected source.
      ...
      where (s.is_admin or upper(trim(ni.uf::text)) = any(s.ufs))
        ...
        and (
          ...
          or ($9 is not null and exists (
            ...
            where m.chave_acesso = ni.chave_acesso
          ))
          or ($10 is not null and ni.chave_acesso = $10)
          or ($9 is null and $10 is null
            and concat_ws(' ', ni.nome_abreviado, ni.estabelecimento,
              ni.nota_fiscal, ni.codigo_cliente) ilike '%%' || $6 || '%%')
        )
      group by
        ni.chave_acesso,
        ni.estabelecimento,
...
comment on function public.listar_nfd_notas_gerencial(
  date, date, text, text, text, text, text, text, integer, integer
) is 'Pagina NFDs gerenciais: agrega apenas os totais da grade para evitar montar detalhes de todas as notas antes da paginacao.';
