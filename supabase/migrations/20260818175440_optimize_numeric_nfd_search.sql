-- Route numeric invoice searches through the existing btree indexes before
-- aggregating nfd_itens into nfd_notas. Text searches retain the previous
-- substring behavior. Rollback requires a forward migration restoring the
-- function definition from 20260812182546_allow_multiple_gerencial_note_locations.
  ...
  execute format($query$
    with scope as materialized (
      ...
      where (s.is_admin or upper(trim(n.uf::text)) = any(s.ufs))
        ...
        and (
          $6 is null
          or ($9 is not null and (n.nota_fiscal = $9 or n.codigo_cliente = $9))
...
comment on function public.listar_nfd_notas_gerencial(
  date, date, text, text, text, text, text, text, integer, integer
) is 'Pagina NFDs gerenciais; pesquisa numerica usa indices de NFD/cliente e chave de acesso.';
